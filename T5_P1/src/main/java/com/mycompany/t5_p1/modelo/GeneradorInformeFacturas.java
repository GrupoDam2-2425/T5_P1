 ...
 * @author David
 */
public class GeneradorInformeFacturas {
    public static void leerInforme(List<Document> listaFacturas){
    try {
            JasperPrint print;
            HashMap parametro = new HashMap();
          
            JRDataSource dataSource = new JRBeanArrayDataSource(listaFacturas.toArray());
            String report = "T5_P1 Facturas_subreportFacturas.jasper";
            print = JasperFillManager.fillReport(report, parametro,dataSource);
            JasperExportManager.exportReportToPdfFile(print, "ListaFacturas.pdf");
